...
<table class="shop_table shop_table_responsive my_account_orders woocommerce-orders-table woocommerce-MyAccount-orders woocommerce-orders-table--orders">
	...
	<thead>
		<tr>
			...
			<th class="order-actions woocommerce-orders-table__header woocommerce-orders-table__header-order-actions"><span class="nobr"><?php echo esc_html_x( 'Actions', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>
	...
	<tbody>
		<?php foreach ( $subscription_orders as $subscription_order ) :
			...
			?><tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?>">
				...
				<!--<td class="order-total woocommerce-orders-table__cell woocommerce-orders-table__cell-order-total" data-title="<?php echo esc_attr_x( 'Link', 'Used in data attribute. Escaped', 'woocommerce-subscriptions' ); ?>">-->
					<?php
					// translators: $1: formatted order total for the order, $2: number of items bought
//					echo '<a href="' . esc_url( $order_received ) . '" class="woocommerce-button button " target="_blank">Add Subscribers</a>';
					?>
				<!--</t d>-->
