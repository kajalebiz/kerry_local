...
add_filter( 'body_class', 'obj_eventcxpa_body_class' );
function obj_eventcxpa_body_class( $classes ) {

	$classes[] = 'eventcxpa-template colored-title';
	return $classes;

}

add_action( 'objectiv_page_content', 'obj_eventcxpa_page_sections' );
function obj_eventcxpa_page_sections() {
	obj_eventcxpa_intro_content();
	/*obj_about_team();
	obj_about_social();
...
function obj_eventcxpa_intro_content() {
	...
	if ( ! empty( $content ) ) {
		...
		obj_event_cxpa_section( $content_section, '', $bg_shapes );
	}
}
