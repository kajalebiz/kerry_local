			...
			if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
				...
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $subscription ) ); ?>">
					...
					<td class="product-name">
						...
						if ( $_product && ! $_product->is_visible() ) {
							echo wp_kses_post( apply_filters( 'woocommerce_order_item_name', $item['name'], $item, false ) );
                                                    ...
                                                    $product_permalink = get_field('page_redirect_link',$item->get_product_id() );
                                                    if( !empty( $product_permalink ) ) {
                                                        $product_permalink = $product_permalink;
                                                    } else {
                                                        $product_permalink = get_permalink( $item['product_id'] );
                                                    }
                                                    echo wp_kses_post( apply_filters( 'woocommerce_order_item_name', sprintf( '<a href="%s">%s</a>', $product_permalink, $item['name'] ), $item, false ) );
						}
					...
					<td class="product-total">
						<?php echo wp_kses_post( $subscription->get_formatted_line_subtotal( $item ) ).' /year'; ?>
					</td>
				</tr>
				<?php
			}
		...
		foreach ( $totals as $key => $total ) :
			...
			<tr>
				<th scope="row" <?php echo ( $allow_item_removal ) ? 'colspan="2"' : ''; ?>><?php echo esc_html( $total['label'] ); ?></th>
                                <td><?php echo wp_kses_post( $total['value'] ); ?> <span class="year-str"><?php echo $year_str;?></span></td>
			</tr>
		<?php endforeach; ?>
	</tfoot>
</table>
