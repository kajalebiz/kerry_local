...
/*
Template Name: Experience Design Bootcamps
 */
...
add_action( 'objectiv_page_content', 'obj_ed_page_sections' );
function obj_ed_page_sections() {
	obj_ed_jump_nav();
	obj_ed_intro_content();
	obj_ed_hyo();
}

function obj_ed_jump_nav() {
...
function obj_ed_intro_content() {
	...
	if ( ! empty( $content ) ) {
		...
		obj_content_section( $content_section, 'section-padding', $bg_shapes );
	}
}

function obj_ed_featured_clients() {
...
function obj_ed_bootcamps() {
	...
	if ( is_array( $event_cats ) ) {
		...
		obj_do_event_cats_side_by_cta_section( $bootcamp_sec_deets, 'bg-light-gray' );
	}
