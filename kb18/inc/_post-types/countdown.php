...
if ( ! function_exists( 'objectiv_countdown_cpt' ) ) {

	// Register Custom Post Type
	function objectiv_countdown_cpt() {

		$labels = array(
			'name'                  => _x( 'Countdowns', 'Post Type General Name', 'text_domain' ),
		...
		$args   = array(
			'label'               => __( 'Countdown', 'text_domain' ),
	...
	add_action( 'init', 'objectiv_countdown_cpt', 0 );

}


// Rename the title text on creating a new post
function objectiv_change_countdown_title( $title ) {
	 $screen = get_current_screen();

	if ( 'countdown' == $screen->post_type ) {
		 $title = 'Name for Countdown Attribution';
	}

	 return $title;
}

add_filter( 'enter_title_here', 'objectiv_change_countdown_title' );
