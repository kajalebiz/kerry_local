...
/*
Template Name: Experience Design Sub Page
 */
...
function obj_journey_development_body_class( $classes ) {
	...
	$classes[] = 'cx-design-sub colored-title';
	return $classes;

}

add_action( 'objectiv_page_content', 'obj_eds_page_sections' );
function obj_eds_page_sections() {
	obj_eds_jump_nav();
	obj_eds_intro_content();
	obj_eds_angled_tabs();
	obj_eds_what_youll_get();
	obj_eds_footer_form();
}

function obj_eds_jump_nav() {
...
function obj_eds_intro_content() {
	$pre_content = get_field( 'first_content_pre_content' );
...
function obj_eds_angled_tabs() {
	...
	if ( ! empty( $tab_content ) ) {
		...
		if ( empty( $tab_content['cta_blocks'] ) ) {
			$tabs_class = 'no-tabs';
		}

		obj_tabbed_cta( $tab_section, 'in-bg-page angled-top white-active white-blurb dark-title ' . $tabs_class );
	}
}

function obj_eds_what_youll_get() {
	...
	$side_by_side_sections = get_field( 'what_youll_get_side_by_side_sections' );
	$lower_content         = get_field( 'what_youll_get_lower_content' );
	...
	$what_you_get_section = array(
		...
		'side_by_side'      => $side_by_side_sections,
